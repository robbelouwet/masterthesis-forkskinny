#ifndef FORKSKINNYPLUS_UTILS_H
#define FORKSKINNYPLUS_UTILS_H

#include <cstdint>

/// f(x) = rotate_right(x, 1) << 8
static inline uint16_t ror_row1_4bit_aligned(uint8_t input) {
	uint16_t values[] = {0x000, 0x800, 0x100, 0x900,
	                     0x200, 0xa00, 0x300, 0xb00,
	                     0x400, 0xc00, 0x500, 0xd00,
	                     0x600, 0xe00, 0x700, 0xf00,
	};
	return values[input];
}

/// f(x) = rotate_right(x, 2) << 4
static inline uint16_t ror_row2_4bit_aligned(uint8_t input) {
	uint16_t values[] = {0x00, 0x40, 0x80, 0xc0,
	                     0x10, 0x50, 0x90, 0xd0,
	                     0x20, 0x60, 0xa0, 0xe0,
	                     0x30, 0x70, 0xb0, 0xf0,
	};
	return values[input];
}

/// f(x) = rotate_right(x, 3)
static inline uint16_t ror_row3_4bit_aligned(uint8_t input) {
	uint16_t values[] = {0x0, 0x2, 0x4, 0x6,
	                     0x8, 0xa, 0xc, 0xe,
	                     0x1, 0x3, 0x5, 0x7,
	                     0x9, 0xb, 0xd, 0xf,
	};
	return values[input];
}

/// f(x⁰x¹x²x³||x⁴x⁵x⁶x⁷) = x²x³x⁰x¹||x⁵x⁶x⁷x⁴, so f(a||b) = rotate_right(a, 2) || rotate_right(b, 3)
static inline uint16_t shiftrows_row2_row3(uint16_t row2row3) {
	
	uint16_t values[] = {0x0, 0x2, 0x4, 0x6, 0x8, 0xa, 0xc, 0xe, 0x1, 0x3, 0x5, 0x7, 0x9,
	                     0xb, 0xd, 0xf, };
	return values[row2row3];
}

static inline uint16_t shiftrows_row0_row1(uint16_t input) {
	uint16_t values[] = {0x0, 0x8, 0x1, 0x9, 0x2, 0xa, 0x3, 0xb, 0x4, 0xc, 0x5, 0xd, 0x6,
	                     0xe, 0x7, 0xf, };
	return values[input];
}

#endif //FORKSKINNYPLUS_UTILS_H
