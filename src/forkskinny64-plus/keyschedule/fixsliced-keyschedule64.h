...
static inline void forkskinny_64_init_tk23_fixsliced_internal(State64Sliced_t *tk1, State64Sliced_t *tk2,
	...
	while (i < FORKSKINNY64_MAX_ROUNDS) {
		...
		out->keys[i++] = rtk12.halves[1];
		forkskinny64_add_constant(&(rtk12.halves[0]), i);
		out->keys[i++] = rtk12.halves[0];
		...
		out->keys[i++] = rtk34.halves[1];
		forkskinny64_add_constant(&(rtk34.halves[0]), i);
		out->keys[i++] = rtk34.halves[0];
		...
		out->keys[i++] = rtk56.halves[1];
		forkskinny64_add_constant(&(rtk56.halves[0]), i);
		out->keys[i++] = rtk56.halves[0];
		...
		out->keys[i++] = rtk78.halves[1];
		out->keys[i++] = rtk78.halves[0];
		...
		out->keys[i++] = rtk910.halves[1];
		forkskinny64_add_constant(&(rtk910.halves[0]), i);
		out->keys[i++] = rtk910.halves[0];
		...
		out->keys[i++] = rtk1112.halves[1];
		forkskinny64_add_constant(&(rtk1112.halves[0]), i);
		out->keys[i++] = rtk1112.halves[0];
		...
		out->keys[i++] = rtk1314.halves[1];
		forkskinny64_add_constant(&(rtk1314.halves[0]), i);
		out->keys[i++] = rtk1314.halves[0];
 ...
 * @return
 */
//static inline KeySchedule64Sliced_t
//forkskinny_64_fixsliced_init_tk2_internal(State64Sliced_t tk1, State64Sliced_t tk2) {
