...
static inline void forkskinny64_encrypt_round(KeySchedule64Sliced_t *schedule, State64Sliced_t *state,
                                              uint16_t iteration) {
	// i: 0, 0x76541200
//	auto roundkey = unslice_accelerated_internal({.halves= {schedule->keys[iteration], {}}}, false).values[0].raw;

//	auto test_sbox_before = unslice_accelerated_internal(*state).values[0].raw; // 0x EFCD AB89 6745 2301
//	auto before0 = _rdtsc();
	forkskinny64_sbox(state);
//	auto after0 = _rdtsc();
//	std::cout << "SBOX: " << after0 - before0 << std::endl;
//	auto test_state = unslice_accelerated_internal(*state).values[0].raw; // 0x 7F4E 5D38 2B1A 90C6
...
//	test_state = unslice_accelerated_internal(*state).values[0].raw; // 0x 7F4E 5D18 C51A 6D26

//	auto before2 = _rdtsc();
	forkskinny64_shiftrows(state);
//	auto after2 = _rdtsc();
//	std::cout << "ShiftRows: " << after2 - before2 << std::endl;
//	test_state = unslice_accelerated_internal(*state).values[0].raw; // 0x F4E7 185D AC51 6D26
...
//	exit(1);
//	test_state = unslice_accelerated_internal(*state).values[0].raw; // 0x 757B B40C 6D26 819C
...
//		auto test = unslice_accelerated_internal(*state).values[0].raw;
//		int bannn = 1;
	}

//	auto test1 = unslice_accelerated_internal(*state).values[0].raw; // 0xE86B7E7E22F3BA92
...
//			auto test2a = unslice_accelerated_internal(*C0).values[0].raw;
//			int appel = 1;
		}
	}

//	auto test2b = unslice_accelerated_internal(*C0).values[0].raw;
	
	// ### C1 ###
	if (mode == '1' || mode == 'b') {
		*C1 = *state;
//		auto test3_before = unslice_accelerated_internal(*C1).values[0].raw;
		add_branch_constant64(C1);
//		u64 test3_bc = unslice_accelerated_internal(*C1).values[0].raw;
...
//			auto test3a = unslice_accelerated_internal(*C1).values[0].raw;
//			int appel = 1;
		}
	}

//	auto test3 = unslice_accelerated_internal(*C1).values[0].raw;
//	int appel = 1;
}
...
//	auto roundkey = unslice_accelerated_internal(uRTK).values[0].raw;

//	auto ct = Blocks64_t{.values = {0x9A9BFD8982C66E7C}};
//	*state = Slice64_t(ct);

//	auto test_sbox_before = unslice_accelerated_internal(*state).values[0].raw;
	forkskinny64_mixcols_inv(state);
//	auto test_state = unslice_accelerated_internal(*state).values[0].raw;
	
	forkskinny64_shiftrows_inv(state);
//	test_state = unslice_accelerated_internal(*state).values[0].raw;
...
//	auto initial_state = unslice_accelerated_internal(*state).values[0].raw;
...
//		test2 = unslice_accelerated_internal(*state).values[0].raw;
//		int appel = 1;
	}
...
//	auto before = unslice_accelerated_internal(*state).values[0].raw;
	add_branch_constant64(state);
//	auto after = unslice_accelerated_internal(*state).values[0].raw;
