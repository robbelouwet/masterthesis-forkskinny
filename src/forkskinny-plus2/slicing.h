 ...
 * @return
 */
static inline uint64_t slice_significance(const Blocks blocks, uint8_t significance) {
...
static inline State64Sliced_t slice(const Blocks blocks) {
 ...
 */
static inline void unslice_significance(const uint64_t slice, Blocks *blocks, uint8_t sb_index) {
...
static inline Blocks unslice(const State64Sliced_t state) {
	Blocks unsliced = Blocks();
	for (int i = 0; i < 64; ++i) {
