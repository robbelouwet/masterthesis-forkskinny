 ...
 * @return
 */
static inline Slice_t slice_significance(const Blocks_t blocks, uint8_t significance) {
...
static inline StateSliced_t slice(const Blocks_t blocks, bool reverse = true) {
 ...
 */
static inline void unslice_significance(const Slice_t slice, Blocks_t *blocks, uint8_t sb_index) {
	#if slice_size == 256
...
static inline Blocks_t unslice(StateSliced_t state, bool reverse_back = true) {
	// reverse the cells (!= 'just' reversing the raw slices array, their order within a cell needs to be maintained!)
//	if (reverse_back)
//		for (int i = 0; i < 8; ++i) {
