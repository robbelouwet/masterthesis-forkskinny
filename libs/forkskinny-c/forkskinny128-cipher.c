...
STATIC_INLINE u64 skinny128_LFSR2(u64 x)
{
    return ((x << 1) & 0xFEFEFEFEFEFEFEFEULL) ^
           (((x >> 7) ^ (x >> 5)) & 0x0101010101010101ULL);
}

STATIC_INLINE u64 skinny128_LFSR3(u64 x)
{
...
STATIC_INLINE u64 skinny128_sbox(u64 x)
{
...
STATIC_INLINE u64 skinny128_inv_sbox(u64 x)
{
    /* See 32-bit version below for a description of what is happening here */
    u64 y;
    x = ~x;
...
//	state.lrow[1] = 0x80;
	...
	/* Apply the subkey for this round */
	#if SKINNY_64BIT
	u64 round_key = schedule1->lrow ^ schedule2->lrow ^ schedule3->lrow;
